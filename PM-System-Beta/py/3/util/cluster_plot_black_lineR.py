...
if __name__ == '__main__':
    ...
    parser.add_argument('--name', type=str, default="cell_cluster", help="sample name", required=False)
    ...
    #tifffile.imwrite(os.path.join(save_dir, name+"_color_img.tif"), cell_color_img, compression="jpeg")

    # 画细胞边界
    outline = cellpose.utils.masks_to_outlines(cells)
    cell_color_img[outline] = [0, 0, 0]
    tifffile.imwrite(os.path.join(save_dir, name+"_color_outline_img.tif"), cell_color_img, compression="jpeg")

    #增加细胞边界dilate
    kernel_size=2  
    kernel = np.ones((kernel_size, kernel_size), np.uint8)  # 5x5 的核
    # 对边界掩码进行膨胀
    gray_outline = (outline.astype(np.uint8) * 255).astype(np.uint8)
    dilated_outline = cv2.dilate(gray_outline, kernel, iterations=1)
    boolean_outline = dilated_outline > 0
    # 在图像上绘制膨胀后的边界
    cell_color_img[boolean_outline] = [0, 0, 0]  # 设置为黑色
    #tifffile.imwrite(os.path.join(save_dir, name+"_color_dilatedoutline_img.tif"), cell_color_img, compression="jpeg")
    #缩小细胞边界erode 
    # 对边界掩码进行腐蚀
    #erosion_kernel = np.ones((3, 3), np.uint8)  # 3x3 的核
    #eroded_outline = cv2.erode(outline, erosion_kernel, iterations=1)
    # 在图像上绘制腐蚀后的边界
    #cell_color_img[eroded_outline] = [0, 0, 0]  # 设置为黑色


    # 加图例
    cell_color_img = add_legend(cell_color_img, legend_pic)
    tifffile.imwrite(os.path.join(save_dir, name+"_with_legend_img.tif"), cell_color_img, compression="jpeg")
    # tifffile.imwrite(os.path.join(save_dir, "cell_cluster_with_legend_img{}.tif".format(sub_index)), cell_color_img, compression="jpeg")
    
    #绘制细胞边界
    #clusterimg_path = os.path.join(save_dir, "cell_cluster_color_outline_img.tif")         
    #img_path = clusterimg_path
    #img = cv2.imread(img_path,0)
    #edges = cv2.Canny(img, 0, img.max())
    '''两类膨胀相交'''
    #cv2.imwrite(os.path.join(save_dir,'edges.tif'),edges)
    #kernel_size=3  
    #kernel = np.ones((kernel_size, kernel_size), np.uint8)
    #dilate = cv2.dilate(edges,kernel=kernel,iterations=1)
    #dilate = dilate>0
    #cv2.imwrite(os.path.join(save_dir,'cluster-8-dilate.tif'),dilate)
