...
# ============================================================
# 默认的 CSV 输入目录
DEFAULT_CSV_ROOT = "E:/xjh/work/辣素/Spatial/16DPA/L7_heAuto/UMAP/object_with_relabel/clusters.csv"
# 默认的 BIN 输出目录
DEFAULT_BIN_ROOT = "E:/xjh/work/辣素/Spatial/16DPA/L7_heAuto/UMAP/object_with_relabel"
# ============================================================
...
def convert_one(csv_path: str, bin_path: str) -> str:
    try:
        ...
        # 2️⃣ 检查必需的列
        need_cols = ["x", "y", "seurat_clusters"]
        if not set(need_cols) <= set(df.columns):
            # 如果没有x,y,cluster列，检查是否存在barcode和cluster列（非空间数据）
            if set(["barcode", "clusseurat_clusterster"]) <= set(df.columns):
                # 对于非空间数据，我们只关心barcode和cluster
                df_out = df[["barcode", "seurat_clusters"]]
                ...
                arr = df[["seurat_clusters"]].to_numpy(np.float32, copy=False)
            else:
        ...
        df["seurat_clusters"] = df["seurat_clusters"].apply(_to_float)
